...
import {
  ...
  HeartPulse, Zap, Star, Scale as ScaleIcon,
} from 'lucide-react';
import { useHapticFeedback } from '@/lib/hooks/useHapticFeedback';

export type BMIFormData = {
  weight: number;
  height: number;
};
...
const DEFAULT_BMI_VALUES: BMIFormData = { weight: 70, height: 175 };

export function BMICalculatorCore({ embed = false, initialValues }: { embed?: boolean; initialValues?: Partial<BMIFormData> }) {
  const effectiveDefaults = useMemo(() => ({ ...DEFAULT_BMI_VALUES, ...initialValues }), [initialValues]);

  const [result, setResult] = useState<BMIResultData | null>(() => {
    try { return calculateBMI(effectiveDefaults); } catch { return null; }
  });
  ...
  const calculateResults = useCallback((data: BMIFormData) => {
  ...
  }, [watchValues, unitSystem, calculateResults]);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Form Section */}
        <div id="bmi-inputs" className="card min-w-0">
