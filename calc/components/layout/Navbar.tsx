...
export function Navbar() {
  ...
  useEffect(() => {
    if (!megaOpen) setMegaSearch('');
  }, [megaOpen]);

  // Auto-focus search input when mega-menu opens
  useEffect(() => {
  ...
  const sq = megaSearch.toLowerCase().trim();
  const filteredFinance = sq
    ? financeCalcs.filter(c =>
        c.title.toLowerCase().includes(sq) ||
        c.description?.toLowerCase().includes(sq) ||
        c.keywords?.some(k => k.toLowerCase().includes(sq))
      )
    : financeCalcs;
  const filteredOther = sq
    ...
    : otherCalcs;
  const megaHasResults = filteredFinance.length > 0 || filteredOther.length > 0;
          ...
          <div className="flex justify-between items-center h-16">
            ...
            >
              ...
              >
                ...
                <rect width="32" height="32" rx="6" fill="url(#navLogoGrad)" />
                <text
                  x="16"
                  y="22"
            ...
            <div className="hidden md:flex items-center gap-1 mx-6">
              ...
              <NavLink
                ...
                active={isActive('/verification-methodology')}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
              ...
              {megaHasResults ? (
                <div className="flex gap-8">
                  ...
                  {/* Vertical divider — only when both sections are visible */}
                  {filteredFinance.length > 0 && filteredOther.length > 0 && (
