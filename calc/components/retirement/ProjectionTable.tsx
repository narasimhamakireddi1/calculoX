...
import { TableProperties, TrendingUp, TrendingDown } from 'lucide-react';
    ...
    <td className="px-4 py-3">
      <span className={`px-2 py-1 rounded text-xs font-semibold ${row.phase === 'accumulation' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300'}`}>
        {row.phase === 'accumulation' ? (
            <span className="inline-flex items-center gap-1"><TrendingUp className="w-3 h-3" strokeWidth={2} aria-hidden="true" /> Accumulating</span>
          ) : (
            <span className="inline-flex items-center gap-1"><TrendingDown className="w-3 h-3" strokeWidth={2} aria-hidden="true" /> Distribution</span>
          )}
      </span>
    </td>
