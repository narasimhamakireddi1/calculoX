...
import { Landmark, GraduationCap, Briefcase, Coins, TrendingUp, Target, Sunrise, BookOpen, Lightbulb, Trash2, Calculator, Calendar, CalendarDays, Clock, HelpCircle, ChevronRight } from 'lucide-react';
...
type SIInputs = { principal: number; annualRate: number; tenureValue: number; tenureType: TenureType };

const DEFAULT_SI_VALUES: SIInputs = { principal: 500000, annualRate: 9, tenureValue: 3, tenureType: 'years' };

function computeSIAll(data: SIInputs) {
  const result = calculateSimpleInterest(data);
  const projections = generateSimpleInterestProjection(data);
  return { result, projections, chartData: projections.slice(0, Math.min(projections.length, 24)), projectionFirstTwelve: projections.slice(0, 12) };
}

const INITIAL_SI_DATA = (() => {
  try { return computeSIAll(DEFAULT_SI_VALUES); } catch { return null; }
})();

export default function SimpleInterestCalculatorPage() {
  const chartColors = useChartColors();
  const [result, setResult] = useState<SIResultData | null>(INITIAL_SI_DATA?.result ?? null);
  const [projections, setProjections] = useState<any[]>(INITIAL_SI_DATA?.projections ?? []);
  const [chartData, setChartData] = useState<any[]>(INITIAL_SI_DATA?.chartData ?? []);
  const [projectionFirstTwelve, setProjectionFirstTwelve] = useState<any[]>(INITIAL_SI_DATA?.projectionFirstTwelve ?? []);
  ...
  return (
    <div className="space-y-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Form Section */}
        <div id="simple-interest-inputs" className="card min-w-0">
          ...
          <form className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Principal amount in Simple Interest. Interest is calculated on this amount without compounding</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Current bank interest rates: 6-7% p.a. for Savings Accounts. Higher for FDs (7-8% p.a.)</p>
            </div>
      ...
      <RelatedBlogPosts posts={getRelatedBlogPosts('/simple-interest-calculator')} />
