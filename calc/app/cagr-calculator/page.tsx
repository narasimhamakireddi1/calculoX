...
import { Shield, TrendingUp, Rocket, BarChart2, BookOpen, Sparkles, Trash2, Lightbulb, Ruler, HelpCircle, ChevronRight } from 'lucide-react';
...
export default function CAGRCalculatorPage() {
  const [result, setResult] = useState<CAGRResultData | null>(() => {
    try { return calculateCAGR({ beginningValue: 100000, endingValue: 200000, years: 5 }); } catch { return null; }
  });
  ...
  return (
    <div className="space-y-8 py-8">
      <div className="text-center">
        <h1 className="text-3xl sm:text-4xl font-bold mb-4 text-gradient flex flex-wrap items-center justify-center gap-2">
          <span className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 shadow-lg flex-shrink-0">
            <CalculatorIcon idOrHref="cagr" className="w-6 h-6 text-white" />
          </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Form */}
        <div id="cagr-inputs" className="card min-w-0">
          ...
          <form  className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Starting investment amount. Use actual investment sum to calculate realistic CAGR</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">₹10,000 to ₹1 Crore</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Final portfolio value after investment period. Must be greater than beginning value for positive CAGR</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Stock market CAGR (Nifty 50): ~13% over 20 years. Long-term investing smooths volatility and improves returns</p>
            </div>
            ...
            <div className="info-panel mt-6">
              ...
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                ...
                <p><strong className="text-gray-900 dark:text-white">Where:</strong> EV = Ending Value | BV = Beginning Value | n = Years</p>
                <p><strong className="text-gray-900 dark:text-white">Result:</strong> Expressed as a percentage (multiply by 100)</p>
              </div>
            </div>
          </form>
        </div>
            ...
            <div id="cagr-results" className="card space-y-4">
              ...
              <div className="stat-tile">
                <p className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide font-semibold mb-1 flex items-center gap-1">
                  <BarChart2 className="w-3 h-3 flex-shrink-0" strokeWidth={2} aria-hidden="true" /> CAGR Factor
                </p>
                <p className="text-sm sm:text-lg font-bold text-emerald-600 dark:text-emerald-400">{result.cagr.toFixed(4)} <span className="text-xs font-normal text-gray-500 dark:text-gray-400">({(result.cagr + 1).toFixed(4)}x growth multiple)</span></p>
              </div>

              {/* Understanding CAGR */}
              <div className="info-panel mt-6">
                <h3 className="font-bold text-gray-900 dark:text-white mb-3 flex items-center gap-2"><BookOpen className="w-4 h-4 flex-shrink-0 text-blue-600 dark:text-blue-400" strokeWidth={2} aria-hidden="true" /> Understanding CAGR</h3>
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                  <p><strong className="text-gray-900 dark:text-white">Beginning Value:</strong> {formatCurrency(watchValues.beginningValue)} - Your initial investment amount</p>
                  <p><strong className="text-gray-900 dark:text-white">Ending Value:</strong> {formatCurrency(watchValues.endingValue)} - Final value after {watchValues.years} year(s)</p>
                  <p><strong className="text-gray-900 dark:text-white">Total Growth:</strong> {formatCurrency(watchValues.endingValue - watchValues.beginningValue)} - Absolute gain from your investment</p>
                  <p><strong className="text-gray-900 dark:text-white">CAGR:</strong> {result.cagrPercentage.toFixed(2)}% - Average annual growth rate (compounded)</p>
                </div>
              </div>
              ...
              <div className="info-panel mt-4">
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                  <p>
                    <strong className="text-gray-900 dark:text-white">Investment Multiplier:</strong> <span className="font-bold text-emerald-600 dark:text-emerald-400">{(watchValues.endingValue / watchValues.beginningValue).toFixed(2)}x</span> - Your money grew by {((watchValues.endingValue / watchValues.beginningValue - 1) * 100).toFixed(1)}%
                  </p>
                  <p>
                    <strong className="text-gray-900 dark:text-white">Comparison Standard:</strong> A CAGR of {result.cagrPercentage.toFixed(2)}% means if you had a fixed-return investment, this % would give the same result
                  </p>
                  <p>
                    <strong className="text-gray-900 dark:text-white">Time Factor:</strong> Over {watchValues.years} year(s), your investment grew at {result.cagrPercentage.toFixed(2)}% annually on average
                  </p>
                  {result.cagrPercentage > 15 && (
                    <p className="text-emerald-700 dark:text-emerald-400 font-semibold">
                      <Lightbulb className="w-3.5 h-3.5 inline mr-1 flex-shrink-0" aria-hidden="true" /><strong>Excellent Return:</strong> A CAGR above 15% is considered very strong for most investments
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="info-panel border-l-4 border-blue-500 mt-4">
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  <strong className="text-gray-900 dark:text-white">Formula:</strong> CAGR = (Ending Value / Beginning Value)^(1/Years) - 1
                </p>
              </div>
              ...
              <div className="info-panel mb-6">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  Your CAGR of <strong className="text-gray-900 dark:text-white">{result.cagrPercentage.toFixed(2)}%</strong> {result.cagrPercentage > 15 ? 'is excellent and beats 95% of Indian equity mutual funds' : result.cagrPercentage > 10 ? 'matches the long-term market average' : 'is below inflation and needs attention'}. Nifty 50 has historically averaged ~13% CAGR over 20+ years. Consider diversified index funds for consistent, tax-efficient long-term growth.
                </p>
              </div>
        ...
        <div className="card">
          ...
          <div className="space-y-4 text-sm">
            <div className="info-panel border-l-4 border-blue-500">
              <p className="font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-1.5"><BarChart2 className="w-3.5 h-3.5 flex-shrink-0 text-blue-600 dark:text-blue-400" strokeWidth={2} aria-hidden="true" /> CAGR (Compound)</p>
              <p className="text-gray-600 dark:text-gray-300">Assumes profits are reinvested each year. Accounts for "interest on interest" (compounding). More accurate for long-term investments.</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2"><strong>Example:</strong> ₹1L → ₹3.1L in 10 years @ 12% CAGR</p>
            </div>
            <div className="info-panel border-l-4 border-gray-300 dark:border-gray-600">
              <p className="font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5 flex-shrink-0 text-gray-400" strokeWidth={2} aria-hidden="true" /> Average Return (Simple)</p>
              <p className="text-gray-600 dark:text-gray-300">Treats each year's return equally. Does NOT account for compounding. Overstates returns for volatile investments.</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2"><strong>Example:</strong> Year1: +50%, Year2: -30% → Avg = 10%, but CAGR = 8.8%</p>
            </div>
          </div>
          <div className="info-panel mt-6">
            <p className="text-sm text-gray-700 dark:text-gray-300">
              <strong className="text-gray-900 dark:text-white">Verdict:</strong> Always use CAGR when evaluating investment performance, especially for periods over 1 year. It gives the true picture of compound growth.
            </p>
          </div>
        </div>
      </div>
      ...
      <RelatedBlogPosts posts={getRelatedBlogPosts('/cagr-calculator')} />
      ...
      <div className="card">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="info-panel">
            <p className="font-bold text-blue-700 dark:text-blue-400 mb-3">What is CAGR?</p>
            <p className="text-sm text-gray-600 dark:text-gray-300">
          ...
          <div className="info-panel">
            <p className="font-bold text-blue-700 dark:text-blue-400 mb-3">Why Use CAGR?</p>
            <p className="text-sm text-gray-600 dark:text-gray-300">
          ...
          <div className="info-panel">
            <p className="font-bold text-blue-700 dark:text-blue-400 mb-3">Good CAGR Range</p>
            <p className="text-sm text-gray-600 dark:text-gray-300">
          ...
          <div className="info-panel">
            <p className="font-bold text-blue-700 dark:text-blue-400 mb-3">Limitations</p>
            <p className="text-sm text-gray-600 dark:text-gray-300">
