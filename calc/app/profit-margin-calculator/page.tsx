...
import { ShoppingBag, Diamond, Package, CheckCircle2, XCircle, Trash2, IndianRupee, BarChart2, Tag, Target, TrendingUp, Percent, Receipt, AlertTriangle, BookOpen, Lightbulb, HelpCircle, ChevronRight } from 'lucide-react';
...
function computeProfitMarginAll(
  data: { costPrice: number; sellingPrice: number; targetMarginPct: number; targetMarkupPct: number; gstRatePct: number },
  options: { calculationBasis: CalculationBasis; gstTreatment: GSTTreatment; marginOrMarkup: 'margin' | 'markup' }
) {
  const result = ProfitMarginGstEngine.calculatePricing(data, options);
  const gstScenarios = ProfitMarginGstEngine.calculateAllGstScenarios(data, {
    calculationBasis: options.calculationBasis,
    marginOrMarkup: options.marginOrMarkup,
  });
  return { result, gstScenarios };
}

const INITIAL_PROFIT_MARGIN = (() => {
  try {
    return computeProfitMarginAll(
      { costPrice: 1000, sellingPrice: 0, targetMarginPct: 20, targetMarkupPct: 0, gstRatePct: 18 },
      { calculationBasis: 'COST_DRIVEN', gstTreatment: 'EXCLUSIVE', marginOrMarkup: 'margin' }
    );
  } catch { return null; }
})();
...
export default function ProfitMarginCalculator() {
  ...
  const [results, setResults] = useState<PricingResult | null>(INITIAL_PROFIT_MARGIN?.result ?? null);
  const [gstScenarios, setGstScenarios] = useState<GstScenario[]>(INITIAL_PROFIT_MARGIN?.gstScenarios ?? []);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const computed = computeProfitMarginAll(
        {
      ...
      setResults(computed.result);
      setGstScenarios(computed.gstScenarios);
    }, 300);
    ...
    <div className="space-y-8 py-8">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Left Column - Inputs */}
          <div id="pricing-inputs" className="lg:col-span-1 card space-y-6 min-w-0">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Your purchase/manufacturing cost. Profit = Selling Price - Cost Price. Higher cost requires higher margin to stay competitive</p>
            </div>
              ...
              <>
                ...
                {watchValues.marginOrMarkup === 'margin' && (
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Healthy retail margins: 20-50% depending on industry. E-commerce: 15-30%, Grocery: 5-15%, Electronics: 10-20%</p>
                  </div>
                )}
                ...
                {watchValues.marginOrMarkup === 'markup' && (
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Markup: increase from Cost. 25% markup on ₹100 = ₹125 selling price. Higher markup = higher margin (non-linear)</p>
                  </div>
                )}
              </>
            )}
            ...
            {watchValues.calculationBasis === 'SELLING_PRICE_DRIVEN' && (
              <div className="space-y-3">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">₹1 - ₹100,000</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Market price or fixed MRP on package. Used in Price-Driven mode when MRP is pre-determined and you need to extract profit</p>
              </div>
            )}
        ...
        <RelatedBlogPosts posts={getRelatedBlogPosts('/profit-margin-calculator')} />
