...
const numberInputClasses =
  'w-full md:w-40 px-2 py-3 text-right [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none border-2 border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none';

const DEFAULT_BVR_INPUT = {
  property_buying_track: { property_value: 8000000, down_payment_pct: 20, loan_interest_rate_pct: 8.5, loan_tenure_years: 20, property_growth_rate_pct: 6, annual_maintenance_pct: 0.5 },
...
function computeBuyVsRentAll(input: Parameters<typeof BuyVsRentEngine.calculate>[0]) {
  const result = BuyVsRentEngine.calculate(input);
  return { result, chartData: result.yearly_data };
}

const INITIAL_BVR_DATA = (() => {
  try { return computeBuyVsRentAll(DEFAULT_BVR_INPUT); } catch { return null; }
})();
...
export default function HomeLoanVsRentCalculator() {
  ...
  const [result, setResult] = useState<BuyVsRentResult | null>(INITIAL_BVR_DATA?.result ?? null);
  const [chartData, setChartData] = useState<YearlyData[]>(INITIAL_BVR_DATA?.chartData ?? []);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      const computed = computeBuyVsRentAll(calculationInput);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12 px-4">
      <div className="max-w-6xl mx-auto">
          ...
          {activeTab === 'property' && (
            <div className="space-y-6">
              ...
              <div className="relative flex-shrink-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Current property prices in metros: ₹50-200L. Higher property value favors renting if rent is low</p>
              </div>
              ...
              <div className="relative flex-shrink-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />20% down payment is typical. Lower down payment → higher EMI. The down payment amount goes to investment in renting scenario</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'loan' && (
            <div className="space-y-6">
              ...
              <div className="relative flex-shrink-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Current home loan rates (2025): 8-9%. Higher rates favor renting. Lower rates favor buying</p>
              </div>
              ...
              <div className="relative flex-shrink-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Key variable: If rent is lower than EMI, renting looks better initially. But EMI is fixed while rent rises each year</p>
              </div>
          ...
          {activeTab === 'assumptions' && (
            <div className="space-y-6">
              ...
              <div className="relative flex-shrink-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Historical (2010-2025): 6-8% p.a. in metros. Higher appreciation favors buying, lower favors renting</p>
              </div>
              ...
              <div className="relative flex-shrink-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Nifty 50 (20y CAGR): 13%. Conservative: 10%, Aggressive: 13-15%. Higher returns favor renting</p>
              </div>
              ...
              <div className="relative flex-shrink-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">0% - 15% per year</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />India's average (2010-2025): 5-6%. Affects rent growth and real returns on investment</p>
              </div>
              ...
              <div className="border-t border-gray-300 dark:border-gray-700 pt-6 mt-6">
                ...
                {watchValues.apply_tax_benefit && (
                  <div className="relative flex-shrink-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex items-start gap-1"><Lightbulb className="w-3 h-3 mt-0.5 flex-shrink-0" aria-hidden="true" />Your tax slab: 20% slab gets ₹40K deduction on ₹2L interest. 30% slab gets ₹60K. Higher tax rates favor buying</p>
                  </div>
                )}
              </div>
        ...
        {result && (
          <>
            {/* Hero Metrics */}
            <div id="home-loan-vs-rent-results" className="space-y-3 mb-8">
              {/* Hero metric */}
              <div
              ...
              >
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-2 font-medium">
                  {result.financial_verdict === 'BUYING_IS_BETTER' ? 'Buying wins over renting' : 'Renting wins over buying'}
                </div>
              </div>
            ...
            <div className={`card mb-8 ${result.financial_verdict === 'BUYING_IS_BETTER' ? 'bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800' : 'bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800'}`}>
              ...
              <div className={`space-y-2 text-sm ${result.financial_verdict === 'BUYING_IS_BETTER' ? 'text-emerald-800 dark:text-emerald-200' : 'text-blue-800 dark:text-blue-200'}`}>
                ...
                <p className="text-xs mt-3">
                  <Lightbulb className="w-3 h-3 inline mr-1 flex-shrink-0" aria-hidden="true" />This analysis assumes constant rates and returns. Your actual outcome depends on market conditions, personal circumstances, and life changes.
                </p>
              </div>
            </div>
        ...
        {!result && <ChartEmptyState columns={2} />}
        ...
        <RelatedBlogPosts posts={getRelatedBlogPosts('/home-loan-vs-rent')} />
      </div>
    </div>
  );
}
