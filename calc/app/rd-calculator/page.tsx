...
import { Rocket, Coins, TrendingUp, BarChart2, BookOpen, Sparkles, Trash2, RefreshCw, Lock, Landmark, Ruler, HelpCircle, Lightbulb, ChevronRight } from 'lucide-react';
...
function computeRDAll(data: RDFormData) {
  const result = calculateRD(data);
  const projections = generateRDProjection(data);
  return { result, projections };
}

const INITIAL_RD_DATA = (() => {
  try { return computeRDAll({ monthlyDeposit: 5000, annualRate: 6, months: 36 }); } catch { return null; }
})();

export default function RDCalculatorPage() {
  const chartColors = useChartColors();
  const [result, setResult] = useState<RDResultData | null>(INITIAL_RD_DATA?.result ?? null);
  const [projections, setProjections] = useState<MonthlyProjection[]>(INITIAL_RD_DATA?.projections ?? []);
  ...
  const calculateResults = (data: RDFormData) => {
    const computed = computeRDAll(data);
  ...
  return (
    <div className="space-y-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Form */}
        <div id="rd-inputs" className="card min-w-0">
      ...
      {result ? (
        <div className="grid lg:grid-cols-2 gap-8">
          {/* Line Chart */}
          <div className="card">
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2"><TrendingUp className="w-6 h-6 flex-shrink-0" strokeWidth={2} aria-hidden="true" /> RD Growth Over Time</h2>
            <ResponsiveContainer width="100%" height={400}>
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="rdAmountGrad" x1="0" y1="0" x2="0" y2="1">
                ...
                />
                <Legend />
                <Area type="monotone" dataKey="amount" stroke="#3b82f6" strokeWidth={2} fill="url(#rdAmountGrad)" name="Total Amount" dot={false} isAnimationActive={false} />
                <Area type="monotone" dataKey="interest" stroke="#10b981" strokeWidth={2} fill="url(#rdInterestGrad)" name="Interest Earned" dot={false} isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          {result && (
            <div className="card">
              <h2 className="text-2xl font-bold mb-6 flex items-center gap-2"><Coins className="w-6 h-6 flex-shrink-0" strokeWidth={2} aria-hidden="true" /> RD Breakup</h2>
              <MemoizedPieChart
                data={[
      ...
      <RelatedBlogPosts posts={getRelatedBlogPosts('/rd-calculator')} />
