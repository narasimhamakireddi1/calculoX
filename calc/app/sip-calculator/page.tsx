...
import { Shield, Rocket, Target, TrendingUp, BarChart2, Coins, BookOpen, Lightbulb, Trash2, RefreshCw, HelpCircle, AlertTriangle, ChevronRight } from 'lucide-react';
...
const INITIAL_SIP_DATA = (() => {
  try {
    return computeSIPAll({ monthlyInvestment: 10000, years: 10, annualReturn: 12, stepUpPercent: 5 });
  } catch {
    return null;
  }
})();
...
export default function SIPCalculatorPage() {
  ...
  const [result, setResult] = useState<SIPResultData | null>(INITIAL_SIP_DATA?.result ?? null);
  const [chartData, setChartData] = useState<ChartDataPoint[]>(INITIAL_SIP_DATA?.chartData ?? []);
  const [projections, setProjections] = useState<YearlyProjection[]>(INITIAL_SIP_DATA?.projections ?? []);
  const [projectionFirstTwelve, setProjectionFirstTwelve] = useState<YearlyProjection[]>(INITIAL_SIP_DATA?.projectionFirstTwelve ?? []);
